...
3. Build the UI (needs [Node.js](https://nodejs.org/) + `pnpm`): `cd frontend && pnpm install && pnpm build`
...
2. Run the build script (it builds the UI via `pnpm` first, then packages with PyInstaller):
...
- `app/` - core application modules (`app/ui/webview/web/` is the built UI - generated by
  `frontend/`, not committed)
- `frontend/` - React + TypeScript UI source (Vite/Tailwind/shadcn)
- `pyproject.toml` - project metadata, dependencies, and linting configuration
- `build.py` - builds the UI, then packages with PyInstaller
